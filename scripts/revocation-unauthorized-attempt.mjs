...
import { buildUnauthorizedRevocationContext } from './capability-common.mjs';

export async function runRevocationUnauthorizedAttemptDemo() {
  const repo = await mkdtemp(path.join(os.tmpdir(), 'rev-demo-unauth-'));
  const host = await CoreHost.create({ repoDir: repo });

  const capability = buildUnauthorizedRevocationContext();
  ...
  return {
    acceptance: 'unauthorized revocation attempt is rejected and capability resolution is denied deterministically',
