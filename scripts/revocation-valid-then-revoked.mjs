...
import { buildRevokedCapabilityContext } from './capability-common.mjs';

export async function runRevocationValidThenRevokedDemo() {
  const repo = await mkdtemp(path.join(os.tmpdir(), 'rev-demo-revoked-'));
  const host = await CoreHost.create({ repoDir: repo });

  const capability = buildRevokedCapabilityContext();
  ...
  return {
    acceptance: 'valid chain becomes denied after deterministic revocation',
