...
cmd_emit_transform_jsonl(){
  atoms="${1:-atoms.jsonl}"
  [ -f "$atoms" ] || die "not a file: $atoms"
  pybin="${PYTHON:-python3}"
  script="$(dirname "$0")/genesis-protocol/tools/genesis_transform.py"
  [ -f "$script" ] || die "transform script not found: $script"
  "$pybin" "$script" "$atoms"
}

cmd_emit_transform_csv(){
  atoms="${1:-atoms.jsonl}"
  [ -f "$atoms" ] || die "not a file: $atoms"
  pybin="${PYTHON:-python3}"
  script="$(dirname "$0")/genesis-protocol/tools/genesis_transform.py"
  [ -f "$script" ] || die "transform script not found: $script"
  "$pybin" "$script" "$atoms" --csv
}

cmd_emit_transform_buffer(){
  atoms="${1:-atoms.jsonl}"
  [ -f "$atoms" ] || die "not a file: $atoms"
  pybin="${PYTHON:-python3}"
  script="$(dirname "$0")/genesis-protocol/tools/genesis_transform.py"
  [ -f "$script" ] || die "transform script not found: $script"
  "$pybin" "$script" "$atoms" --buffer
}
...
USAGE
  ...
  genesis.sh emit-transform-jsonl  [ATOMS.jsonl]
  genesis.sh emit-transform-csv    [ATOMS.jsonl]
  genesis.sh emit-transform-buffer [ATOMS.jsonl]
...
TRANSFORM
  The emit-transform-* commands apply the canonical GENESIS transform
  (RFC-GENESIS-TRANSFORM-0001) to convert atoms to 3D coordinates:
    - emit-transform-jsonl: Enhanced JSONL with transform fields
    - emit-transform-csv:   CSV with position/scale/rotation
    - emit-transform-buffer: Binary Float32 buffer (5 floats/atom)
EOF
}
