# DBC / HD-RPC / NRR Specification Index
...
NRR-1.0                             ← substrate (blobs · log · replay · integrity · bundles)
                                       ↑ circular closure: new meaning returns to substrate
```
...
| `NRR-1.0` | Substrate: content-addressed storage, append-only log, deterministic replay | Runtime authors, embedded implementers |
...
**Substrate / embedded implementer:** NRR-1.0 only (no other dependency)

**Node operator:** NRR-1.0 → DBC-1.2 → DBC-IDL-1.2 → DBC-NODE-1.0 → HD-RPC-1.0-DEPLOYMENT (Profile A)
...
**GUI implementer:** HD-RPC-GUI-1.0 → HD-RPC-EVR-1.0 → DBC-NODE-1.0 (endpoint contracts)

**Device / IoT integrator:** NRR-1.0 §11 → HD-RPC-1.0 §8 → HD-RPC-1.0-DEPLOYMENT (device section) → HD-RPC-EVR-1.0 Family G

**Conformance test author:** NRR-1.0 §13 → DBC-1.2 → DBC-1.2-FIXTURES → DBC-TEST-MATRIX → DBC-CONFORMANCE-MATRIX
...
| Law | Spec |
|---|---|
| Content Address Law | NRR-1.0 |
| Append-Only Law | NRR-1.0 |
| Replay Law | NRR-1.0 |
| Artifact Immutability Law | NRR-1.0 |
| Transport Neutrality Law | NRR-1.0 |
| Authority Neutrality Law | NRR-1.0 |
...
| Artifact Immutability (node) | DBC-NODE-1.0 |
...
| Transport Neutrality (HD-RPC) | HD-RPC-1.0 |
